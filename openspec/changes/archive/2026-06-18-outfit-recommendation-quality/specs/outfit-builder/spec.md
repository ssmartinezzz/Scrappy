...
(Previously: calzado eligibility used a single hardcoded footwear whitelist with no style concept; this is replaced by the style-keyed rule below.)
...
(Previously: fallback applied only after price/genero filtering, with no concept of style eligibility or feedback exclusion as additional pre-filters.)
