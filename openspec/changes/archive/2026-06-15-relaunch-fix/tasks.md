...
- [x] Build passes: `mvn -f scraper/pom.xml clean package -DskipTests`
- [x] `POST /api/scrape?forceRetrain=false` → same behavior as today (24h gate active)
- [x] `POST /api/scrape?forceRetrain=true` → `[ML-TRAIN] ===== Iniciando entrenamiento =====` appears in log regardless of model age
- [x] During scraping run, console shows "Procesando y agregando resultados..." and "Entrenando modelo ML en background..." as status messages
- [x] `DELETE /api/db/productos` while IDLE → 200; subsequent `GET /api/data` returns `[]`
- [x] `DELETE /api/db/ml` while IDLE → 200; subsequent `GET /api/tendencias` returns empty/null
- [x] `DELETE /api/db/productos` while RUNNING → 409 with message
- [x] Frontend "Borrar catálogo" button shows confirm dialog; canceling makes no API call
- [x] Frontend "Borrar catálogo" button shows success message after 200
- [x] `ml_pipeline.py` loads models from correct directory on second run
