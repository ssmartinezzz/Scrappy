...
(Previously: explicitly forbade using feedback to alter sampling; that constraint is reversed by this change.)
...
(Previously: forbade any counter-based or co-occurrence weighting of feedback into generation; that constraint is reversed for marca+categoria pair weighting, with time-decay and per-user personalization remaining explicitly out of scope.)
