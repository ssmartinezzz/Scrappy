...
| Catálogo | GET `/api/data` · `/api/facets` · `/api/csv` · `/api/producto?url=` (producto + historial) · DELETE `/api/data?url=` (soft-delete) |
...
Outfits `/outfits` · Historial de precios `/historial?url=`. `/tendencias` redirige a `/analisis/mercado`.
